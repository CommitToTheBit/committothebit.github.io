...
So yes, profile-guided optimisations are a deal with the devil, and like any deal with the devil you'd be well-advised to read the small print (read: the documentation). Their saving grace is, if things go wrong, you can literally just turn the `-fprofile-use` flag back off and move on with your life; these optimisations are entirely reversible. If you too would like to live deliciously then I heartily, heartily recommend you give PGO a try, but if it sucks... **hit da bricks!!**