...
Like I say, profile-guided optimisations are a deal with the devil, and like any deal with the devil you'd be well-advised to read the small print (read: the documentation). The saving grace is, if things go wrong - you can literally just turn them off. So long as you **measure, measure, measure** performance before and after adding an optimisation, you will catch any regression PGO causes. If you can't then fix those regressions, you can simply turn the `-fprofile-use` flag and move on with your life, your only real cost being the time wasted collecting telemtry data. All of which is to say: ask yourself whether you too would like to live deliciously, give PGO a try, and if it sucks... **hit da bricks!!**