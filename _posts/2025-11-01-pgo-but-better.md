...
By improving the performance of instrumented builds, we improve the quality of the resulting profiles. The observer effect I mentioned earlier, that's what happens when instrumentation creates bottlenecks where they don't already exist, or where it slows a program down to the point it takes a different execution path than it would otherwise. Misidentifying hot paths for cold paths means optimising them for size rather than speed, and vice versa (though in game development the first case is generally more severe).
